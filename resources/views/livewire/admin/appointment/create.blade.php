<div>
    ...
    <div class="relative md:min-h-[20px] md:h-16 bottom-0 left-0 right-0 flex flex-row p-4 md:bg-white mb-8">
        ...
        <h1 class="relative z-10 ml-3 text-xl font-semibold tracking-tight text-gray-700">Create Appointment</h1>
        <div class="absolute inset-0 shadow-lg"></div>
    </div>
    <div class="w-4/5 p-5 m-auto mt-10 h-3/4 min-w-[500px]">
        <x-card title="Create New Appointment" rounded="3xl" id="custom-shadow-x-card">
            <form wire:submit.prevent="createAppointment">
                <div>
                    <x-datetime-picker label="Appointment Date" placeholder="Appointment Date"
                        wire:model.lazy="date_time" :min="now()->addHours(2)" />
                </div>
                <div>
                    <x-select label="Search a User" wire:model="user_id" placeholder="Select some user"
                        :async-data="route('patient.references')" :template="[
                            'name' => 'user-option',
                            'config' => ['src' => 'profile_photo'],
                        ]" option-label="last_name" option-value="user_id"
                ...
                {{-- <div>
                    <x-select
                        ...
                        :async-data="route('service.references')"
                        option-label="name"
                        option-value="id"
                    />
                </div> --}}
                ...
                <div class="flex flex-row pt-3 gap-x-3">
                    <x-select class="mb-3" label="Select Services" wire:model.lazy="service_id" multiselect
                        placeholder="Select services" :async-data="route('service.references')" option-label="name" option-description="price"
                        option-value="id" />
                </div>
                <div>
                    <x-textarea label="Concern" placeholder="Describe your dental health concern"
                        wire:model.defer='concern' />
                </div>
                <x-slot name="footer" class="flex items-center justify-between">
                    <div class="justify-end">
                        <x-button class="outline" label="Cancel" flat href="{{ route('appointment.index') }}"
                            target="_self" />
                        <x-button indigo label="Save" wire:click='createAppointment' wire:loading.attr="disabled" />
                    </div>
                </x-slot>
            </form>
        </x-card>
