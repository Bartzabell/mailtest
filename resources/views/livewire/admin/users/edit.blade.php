<div>
    ...
    <div class="relative md:min-h-[20px] md:h-16 bottom-0 left-0 right-0 flex flex-row p-4 md:bg-white mb-8">
        ...
        <h1 class="relative z-10 ml-3 text-xl font-semibold tracking-tight text-gray-700">Edit Users</h1>
        <div class="absolute inset-0 shadow-lg"></div>
    </div>
    <div class="w-4/5 p-5 m-auto mt-10 h-3/4 min-w-[500px]">
        <x-card title="User Patient" rounded="3xl" id="custom-shadow-x-card">
            <div>
            ...
            <div class="flex mb-5">
                <div class="w-1/3">
                    <x-select class="p-3" label="Select Role" placeholder="Select Role" :options="[
                        ['label' => 'Dentist', 'name' => 'admin'],
                ...
                <div class="w-1/3">
                    <x-select class="p-3" label="Select Status" placeholder="Select one status" :options="[
                        ['name' => 'Active', 'id' => 1],
                        ['name' => 'Inactive', 'id' => 0],
                        // ['name' => 'Stuck', 'id' => 3],
                        // ['name' => 'Done', 'id' => 4],
                    ]"
                        option-label="name" option-value="id" wire:model.lazy="status" />
                </div>
            </div>

            <div>
                <x-inputs.password class="p-2" label="New Password" wire:model.lazy="password" />
            </div>
            ...
            <x-slot name="footer" class="flex items-center justify-between">
                <div class="justify-end">
                    ...
                    <x-button indigo label="Save" wire:click='saveConfirmation' />
                </div>
            </x-slot>
        </x-card>
    </div>
