<div  class="flex flex-col min-h-screen">
    ...
    <div class="flex flex-col justify-between gap-2 lg:flex-row">
        <div class="flex flex-col">
            <div class="w-auto">
                <div class="flex flex-col w-auto max-w-screen-lg bg-white shadow-md">
                    <div class="flex flex-col h-auto pl-10 pr-8 mx-auto mt-4 lg:px-5 lg:w-72 md:w-full">
                        <div class="relative w-24 h-24 mx-auto mb-1 mr-5 overflow-hidden bg-white border-2 border-gray-200 rounded-full fill-black pr-7 lg:w-36 lg:h-36">
                            ...
                            @else
                                @if(strtolower($patientData->sex) === 'male')
                                    <svg class="absolute inset-0 w-full h-full pr-4 mt-4" fill="black" viewBox="0 0 384 512" xmlns="http://www.w3.org/2000/svg">
                                ...
                                @elseif(strtolower($patientData->sex) === 'female')
                                    <svg class="absolute inset-0 w-full h-full pr-3 mt-3 lg:pr-4 lg:mt-4" fill="black" viewBox="0 0 384 512" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div class="w-full px-2 my-1 text-lg text-center text-gray-500">
                            {{ auth()->user()->patientsData->last_name }},
                            {{ auth()->user()->patientsData->first_name }} {{ auth()->user()->patientsData->middle_name }}.
                            {{ auth()->user()->patientsData->extension_name }}</div>
                    ...
                    <div class="flex flex-col h-full">
                        <div class="flex flex-col px-4 py-2 lg:py-4 md:flex-row">
                            {{-- Date of Birth --}}
                            <div class="flex flex-col w-full border-gray-300 lg:border-r md:mb-0 md:w-28 lg:w-48 lg:mr-2">
                                <label class="flex items-center text-sm">Birthdate:</label>
                                <div class="h-auto text-gray-500">
                                    {{ Carbon\Carbon::parse(auth()->user()->patientsData->birthdate)->format('F d, Y') }}
                                </div>
                            </div>

                            {{-- Age --}}
                            <div class="flex flex-col w-full border-gray-300 lg:border-r md:w-16 lg:w-36 md:mb-0 lg:mr-2">
                                <label class="flex items-center text-sm">Age:</label>
                                <div class="text-gray-500">
                                    {{ Carbon\Carbon::parse(auth()->user()->patientsData->birthdate)->diff(Carbon\Carbon::now())->format('%y years') }}
                                </div>
                            </div>

                            {{-- Sex --}}
                            <div class="flex flex-col w-full md:w-16 lg:w-32">
                                <label class="flex items-center text-sm">Sex:</label>
                                <div class="text-gray-500">{{ auth()->user()->patientsData->sex }}</div>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col px-4 py-2 lg:py-4 md:flex-row">
                            ...
                            <div class="flex flex-col w-full border-gray-300 lg:border-r md:mb-0 md:w-28 lg:w-48 lg:mr-2">
                                ...
                                <div class="h-auto text-gray-500">
                                    {{ auth()->user()->patientsData->barangay->barangay_description }}</div>
                            </div>
                            {{-- City --}}
                            <div class="flex flex-col w-full border-gray-300 lg:border-r md:w-16 lg:w-36 md:mb-0 lg:mr-2">
                                <label class="flex items-center text-sm">City:</label>
                                <div class="text-gray-500">
                                    {{ auth()->user()->patientsData->city->city_municipality_description }}</div>
                            </div>
                            {{-- Province --}}
                            <div class="flex flex-col w-full md:w-20 lg:w-32">
                                <label class="flex items-center text-sm">Province:</label>
                                <div class="text-gray-500">
                                    {{ auth()->user()->patientsData->province->province_description }}</div>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col px-4 py-2 lg:py-4 md:flex-row">
                            {{-- Email --}}
                            <div class="flex flex-col w-full border-gray-300 lg:border-r md:mb-0 md:w-28 lg:w-52 lg:mr-2">
                                <label class="flex items-center text-sm">Email:</label>
                                <div class="h-auto text-gray-500">{{ auth()->user()->email }}</div>
                            </div>
                            {{-- Phone Number --}}
                            <div class="flex flex-col w-full md:mb-0 md:w-28 lg:w-32 lg:mr-2">
                                <label class="flex items-center text-sm">Contact Number:</label>
                                <div class="text-gray-500">{{ auth()->user()->patientsData->phone_number }}</div>
                            </div>
                        ...
                        <div class="flex flex-col px-4 py-2 lg:py-4 md:flex-row">
                            ...
                            <div class="flex flex-col w-full md:w-20 lg:w-52 md:mb-0 lg:pr-3">
                                <label class="flex items-center text-sm">Existing Medical Condition:</label>
                                <div class="h-auto text-gray-500">
                                    {{ auth()->user()->patientsData->existing_medical_condition }}</div>
                            </div>
                ...
                <div>
                    <div class="p-6 m-auto mt-3 mb-4 text-base bg-white rounded-lg ">
                        ...
                        <div class="flex flex-row pt-3 gap-x-3">
                            <x-select class="mb-3" label="Services Rendered" wire:model.lazy="servicesId" multiselect
                            ...
                            <x-button class="ml-12 text-white rounded-lg shadow-lg bg-gradient-to-r from-blue-700 via-blue to-blue-300 hover:text-black hover:scale-105"
                                label="Add Services Details" icon="plus"
            ...
            @if ($addPatientRecord == 1)
                <div>
                    <div class="p-6 m-auto mt-3 mb-6 text-base bg-white rounded-lg ">
                        ...
                        <div>
                            <x-datetime-picker class="py-2 pl-3" label="Date and Time Rendered" placeholder="Select Date and Time"
                                wire:model.defer="dateTimeRendered" :max="now()" />
                        </div>
                        <div class="flex flex-row pt-3 gap-x-3">
            ...
            @foreach ($sortedRecords as $record)
                <div class="w-full py-4 pl-5 mt-4 ml-0 bg-white border-2 border-black shadow-md rounded-xl hover:shadow-lg">
                    {{-- date rendered --}}
                    <div class="flex flex-col gap-3">
                        @if ($record->pdrDetail->isnotEmpty())
                            <p class="text-sm text-gray-500 md:text-base">{{ Carbon\Carbon::parse($record->pdrDetail[0]->date_time_rendered)->format('F d, Y H:i A') }}
                            </p>
                        @else
                        ...
                        @if ($record->dentistData->count() > 0)
                            <p class="text-sm text-gray-500 md:text-base">Dr. {{ $record->dentistData->last_name }}</p>
                        @else
                        ...
                        @endif
                            ...
                            {{-- @foreach ($record->pdrDetail as $detail)
                                @if ($detail->services)
                                    ...
                                    @endforeach
                                @else
                                    <p class="text-sm text-gray-500 md:text-base">No services associated with this detail</p>
                                @endif
        ...
        <div class="w-full shadow-lg lg:w-1/2 border-1 border-lg" style="width: 58%; height: 58%;">
